...
public class UserRestController {
	...
	// les mots de passe seront encodés dès création d'une personne (formateur,
	// stagiaire, etc.)
	@Autowired
...
//CREATE pour: 
	...
	public ResponseEntity<Void> insert(@Valid @RequestBody Formateur formateur, BindingResult br,
		...
		// on sauvegarde ds la base avec le mdp encrypté
...
//UPDATE pour: 
	...
	public ResponseEntity<Void> update(@PathVariable(name = "id") Integer id, @Valid @RequestBody Formateur formateur,
		...
		if (br.hasErrors()) {
			...
			if (opt.isPresent()) {
				...
				formateur.setId(id);
				// fBdd reset id et version avant sauvegarde ?
	...
	public ResponseEntity<Void> update(@PathVariable(name = "id") Integer id,
			@Valid @RequestBody Gestionnaire gestionnaire, BindingResult br) {
		if (br.hasErrors()) {
			return new ResponseEntity<Void>(HttpStatus.BAD_REQUEST);
		} else {
			Optional<Gestionnaire> opt = daoGestionnaire.findById(id);
	...
	public ResponseEntity<Void> update(@PathVariable(name = "id") Integer id,
			@Valid @RequestBody Administrateur administrateur, BindingResult br) {
		if (br.hasErrors()) {
			return new ResponseEntity<Void>(HttpStatus.BAD_REQUEST);
		} else {
			Optional<Administrateur> opt = daoAdministrateur.findById(id);
